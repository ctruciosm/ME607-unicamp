---
title: "Análise exploratória de dados I"
...
- EDA permitirá que identifiquemos padrões, dados atípicos, mudanças de comportamento em diferentes períodos, etc. Isto todo nos ajudara a escolher como abordar o problema.
...
1. A definição de $\hat{\gamma}(k) = n^{-1} \displaystyle \sum_{t = 1}^{t-k} (y_{t + k} - \bar{y})(y_t - \bar{y})$ é preferida (em lugar de utilizar $(n - k)^{-1}$) pois essa definição faz com que a função seja semidefinida positiva o que garante que a variância de combinações lineares de $y_t$ seja sempre não negativa.
...
- Pode-se provar que $$\phi_{kk} = \begin{vmatrix}
  ...
  \end{vmatrix}  \begin{vmatrix}
    1 & \rho_1 & \rho_2 & \cdots & \rho_{k-2} & \rho_{k-11}\\
...
- Séries estacionárias são caracterizadas pela média $\mu$, variância $\sigma^2$, autocorrelações $\rho(k)$ e autocorrelações parciais $\phi_{kk}$.
...
Um processo $\{ \epsilon_t \}$ é chamado de ruido branco se é uma sequência de variáveis aleatório da distribuição fixa com $\mathbb{E}(\epsilon_t) = \mu$ (usualmente 0), $\mathbb{V}(\epsilon_t) = \sigma^2$ e $\gamma(k) = 0 \quad \forall k$.
