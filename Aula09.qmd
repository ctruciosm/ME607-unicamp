...
> Até agora tudo tem sido calculado utilizando o modelo teórico (*i.e.* assumindo que conhecemos os verdadeiros valores dos parâmetros). Na prática os parâmetros nunca são observados, precisando ser estimados. **Na aula de hoje aprenderemos como estimar estes parâmetros**.
...
#### Observação
Para ganhar os pontos, deverá verificicar que sua implementação está correta. Para isto, faça um estudo de simulação (reporte o vies e rmse). Ademais, deverá verificar também se os intervalos de confiança tem uma cobertura próxima da nominal.
:::

Trazer um relatório de uma ou duas página contendo: (a)código, (b)tabelas (ou gráficos) com os resultados da simulação e (c) um exemplo de como utilizar.
