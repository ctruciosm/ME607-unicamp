...
- $\vdots$
- $\mathbb{E}_T(Y_{T+h}) = c \times  \sum_{i = 0}^{h-1}\phi^i + \phi^hT_{T}$
...
- $\mathbb{E}_T(Y_{T+h})$
- $\mathbb{V}_T(Y_{T+h}) = \mathbb{V}(e_T(h))$
...
> Na prática, a cada nova observação os parâmetros são re-estimados e as previsões são feitas novamente. Assim, quando caalcularmos o erro de previsão deve-se considerar também o erro devido à estimação
