...
```{r}
#| echo: true
modelo <- lm(y ~ seasonal + trend + I(trend^2), data = USgas_df)
summary(modelo)
```
...
```{r}
#| echo: true
USgas_df$yhat <- predict(modelo, newdata = USgas_df)
...
ggplot(USgas_df) +
  ...
  xlab("Year") +
  ggtitle("Observed and fitted USgas values")
...
#| echo: true
library(forecast)
modelo_tslm <- tslm(USgas ~ season + trend + I(trend^2))
summary(modelo_tslm)    
```
...
<h4>Produção trimestral de cerveja na Australia</h4>
...
- Uma das vantagens de utilizar modelos de regressão num contexto de séries temporais, é sua flexibilidade para lidar com modelos complexos.
- Por exemplo, no seguinte exercícios, trabalharemos com multiplos períodos sazonais.
- A série `UKgrid` do pacote `UKgrid`, contém dados da demanda nacional de energia de 30 em 30 minutos no Reino Unido (`ND`).
    *   Com o aprendido até agora, construir um modelo para prever as próximas 365 observações (poucos mais de 7 dias) 
